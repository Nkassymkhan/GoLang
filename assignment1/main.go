...
func showAllItems() {
	...
	rows, err := db.Query(`SELECT name, price, rating FROM items where quantity > 0`)
	CheckError(err)
...
func filterItemsByPrice() {
	rows, err := db.Query(`SELECT name, price, rating FROM items where quantity > 0 order by price`)
	CheckError(err)
...
func filterItemsByRating() {
	rows, err := db.Query(`SELECT name, price, rating FROM items where quantity > 0 order by rating`)
	CheckError(err)
...
func searchItemByName() {
	...
	var find string
	fmt.Scanln(&find)
	rows, err := db.Query(`SELECT name, price, rating FROM items where quantity > 0 and name = $1`, find)
	CheckError(err)
	...
	for rows.Next() {
		...
		err = rows.Scan(&name, &price, &rating)
		CheckError(err)
		fmt.Printf(" %-15s  %-15s %-15s \n", "name", "price", "rating")
		fmt.Printf(" %-15s  %-15f %-15f \n", name, price, rating)
		noRow = true
...
func GiveRating(name string) {
	fmt.Println("Rate an item from 0 to 5")
	var neww float32
	fmt.Scanln(&neww)
	rows, err := db.Query(`SELECT rating FROM items where name = $1`, name)
	CheckError(err)
	defer rows.Close()
	for rows.Next() {
		var raiting float32
		...
		_, err = db.Exec(`update items set rating = $1 where name = $2`, raiting, name)
		CheckError(err)
