...
func showall(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "<table>")
	fmt.Fprintf(w, " <h2> All items in items </h2><tr><th>Name</th><th>Price</th><th>Raiting</th></tr>")
	for _, i := range data {
...
func filterItems(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "<table>")
	fmt.Fprintf(w, " <body><h2> Filtering by price </h2> <tr><th>Name</th><th>Price</th><th>Raiting</th></tr></body>")
	for _, i := range data {
		fmt.Fprintf(w, "<tr><td>%s</td><td>%v</td><td>%v</td></tr>", i.Name, i.Price, i.Raiting)
	}
	fmt.Fprintf(w, "</table>")
...
func giveRating(w http.ResponseWriter, r *http.Request) {
	...
	if id == "" || newValue == "" {
		fmt.Fprint(w, "ID or value can not be empty")
	}

	stmt, err := db.Prepare("UPDATE items SET rating = (rating + $1)/2 WHERE id = $2")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
	defer stmt.Close()
	...
	http.Redirect(w, r, "/templates/update.html", http.StatusSeeOther)
}
